...
export interface FinanceCommandResult {
  reply: string;
  action: Exclude<CommandAction, 'passthrough' | 'create'>;
}
...
const commandSchema = {
  ...
  required: ['action', 'target', 'changes', 'confidence']
} as const;
      ...
      messages: [
        {
          ...
          content: [
            ...
            '如果用户是新增记账，无论表达方式多自然，都返回 create，让现有多笔 intake 解析器处理。',
            '如果用户要求分析/建议/比较，不在本层处理，返回 passthrough。',
            `参考本地时间：${referenceLocalNow}。`,
            financeReferencePrompt(catalog)
          ].join('\n')
        },
        { role: 'user', content: text }
      ],
      response_format: { type: 'json_schema', json_schema: commandSchema }
    });
...
async function resolveTargets(
  ...
  const count = command.target.scope === 'latest'
    ? 1
    : command.target.count > 0
  ...
  const result = await env.DB.prepare(`
    ...
    LIMIT ?
  `).bind(...binds, count).all<TransactionRow>();
...
async function executeUpdate(
  ...
  for (const row of selected) {
    ...
    `).bind(
      ...
      nextCategoryId,
      nextAccountId,
      command.changes.merchant || row.merchant,
      command.changes.description || row.description,
      command.changes.occurred_at || row.occurred_at,
      row.id
    ));
  }

  const afterRows: TransactionRow[] = selected.map((row) => ({
    ...row,
    type: command.changes.transaction_type || row.type,
    amount_fen: command.changes.amount > 0 ? Math.round(command.changes.amount * 100) : row.amount_fen,
    merchant: command.changes.merchant || row.merchant,
    description: command.changes.description || row.description,
    occurred_at: command.changes.occurred_at || row.occurred_at
  }));
  ...
  return {
    action: 'update',
    reply: selected.length === 1 ? `已修改：${rowLine(afterRows[0])}` : `已修改 ${selected.length} 笔账目。`
  };
}
...
async function executeRestore(
  ...
  const count = command.target.count > 0 ? command.target.count : 1;
  const ops = await env.DB.prepare(`
    ...
    LIMIT ?
  `).bind(Math.min(20, count)).all<{ id: string; before_json: string | null }>();

  const candidates: TransactionSnapshot[] = [];
  for (const op of ops.results || []) candidates.push(...parseSnapshots(op.before_json));
  ...
  if (!restored.length) return { action: 'restore', reply: '最近删除的账目已经恢复过了，没有重复写入。' };
  statements.push(await auditStatement(env, 'restore', source, sourceId, command, restored.map((s) => s.transaction.id), null, restored));
  await env.DB.batch(statements);
  const total = restored.reduce((sum, snap) => sum + Number(snap.transaction.amount_fen || 0), 0);
  return {
    action: 'restore',
    reply: restored.length === 1
      ? `已恢复：${rowLine(restored[0].transaction)}`
      : `已恢复 ${restored.length} 笔，共 ¥${(total / 100).toFixed(2)}。`
  };
}
...
export async function handleFinanceCommandTelegram(
  ...
  if (!command || command.confidence < 0.6 || command.action === 'passthrough' || command.action === 'create') return null;
